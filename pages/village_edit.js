    ...
    <App>
      {!slug && !id && (
        <Mutation mutation={SAVE_VILLAGE}>
          {(
            saveVillage,
            { error: errorSaveVillage, client: clientSaveVillage }
          ) => {
            return (
              <VillageForm update={saveVillage} client={clientSaveVillage} />
            )
          }}
        </Mutation>
      )}
            ...
            <Mutation mutation={SAVE_VILLAGE}>
              {(
                ...
                <Mutation mutation={REMOVE_VILLAGE}>
                  {(
                    removeVillage,
                    { error: errorRemoveVillage, client: clientRemoveVillage }
                  ) => {
